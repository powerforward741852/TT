...
+(NSString *)spellUrl:(NSString *)tempUrl
{
    ...
    JLLog(@"mytoken = %@",STUserDefaults.token);
 ...
 *    看完广告回调
 */
+ (void)WatchADBackDataWithParam:(NSString *)articid WithDataBlock:(sendData)sendBlock
{
    STAFNetWorking *networking = [[STAFNetWorking alloc]init];
 ...
 *    分享应用回调
 */
+ (void)ShareAppBackDataWithParam:(NSString *)articid  WithDataBlock:(sendData)sendBlock
{
    STAFNetWorking *networking = [[STAFNetWorking alloc]init];
    NSDictionary *params = @{
                             @"artid":articid,
                             };
    [networking requsetWithPath:[self spellUrl:@"sharerecover.php?"] Withparams:params withRequestType:NetworkPostType withResult:^(id responseObject, NSError *error) {
